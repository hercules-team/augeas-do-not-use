...
redis-server.
*)
let standard_entry =  [ indent . key k . del_ws_spc . Quote.do_quote_opt (store v) . eol ]

let save = /save/
let seconds = [ label "seconds" . Quote.do_quote_opt (store Rx.integer) ]
let keys = [ label "keys" . Quote.do_quote_opt (store Rx.integer) ]
(* View: save_entry
...
let slaveof = /slaveof/
let ip = [ label "ip" . Quote.do_quote_opt (store Rx.ip) ]
let port = [ label "port" . Quote.do_quote_opt (store Rx.integer) ]
(* View: slaveof_entry
...
let from = [ label "from" . Quote.do_quote_opt (store Rx.word) ]
let to = [ label "to" . Quote.do_quote_opt (store Rx.word) ]
(* View: save_entry
